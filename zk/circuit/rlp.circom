...
template Rlp() {
    ...
    finalNoncePrefix <== isSingleNonceBytePrefix + extendedNoncePrefix + nonceIsZero.out * 0x80;
    ...
    component balanceIsZero = IsZero();
    balanceIsZero.in <== balance;

    isSingleBalanceBytePrefix <== balanceIsSingleByte * balance;
    extendedBalancePrefix <== (1 - balanceIsSingleByte) * (0x80 + bytelb.len);
    finalBalancePrefix <== isSingleBalanceBytePrefix + extendedBalancePrefix + balanceIsZero.out * 0x80;
